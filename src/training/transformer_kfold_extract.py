"""
Inference-only script — extract holdout and test probas from saved k-fold checkpoints.

Run this once after transformer_kfold.py has completed. Outputs:
  models/transformer_kfold/ho_proba.npy   — shape (1790,)  ensemble holdout probas
  models/transformer_kfold/ho_idx.npy     — shape (1790,)  original df indices (for alignment check)
  models/transformer_kfold/test_proba.npy — shape (3836,)  ensemble test probas

These files are consumed by stacking.py for Experiment 3 (late fusion).
Takes ~2 min on RTX 5070 (inference only, no training).
"""
...
# ============================================================
# Paths  (must match transformer_kfold.py)
# ============================================================
...
DATA_DIR   = _root / "data"
MODEL_DIR  = _root / "models" / "transformer_kfold"

if not MODEL_DIR.exists():
    raise FileNotFoundError(f"k-fold model dir not found: {MODEL_DIR}\nRun transformer_kfold.py first.")


# ============================================================
# Config  (must match transformer_kfold.py exactly)
# ============================================================
MODEL_NAME  = "microsoft/deberta-v3-small"
MAX_LENGTH  = 128
BATCH_SIZE  = 32       # inference only — larger batch is fine
K_FOLDS     = 5
SEED        = 42
CLS_DROPOUT = 0.3
...
# ============================================================
# Input formatter  (must be identical to transformer_kfold.py)
# ============================================================
...
# ============================================================
# Data — same splits as transformer_kfold.py
# ============================================================
