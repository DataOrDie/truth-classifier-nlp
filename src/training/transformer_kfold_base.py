"""
Transformer fine-tuning — Experiment 4: DeBERTa-v3-base upgrade.

Identical to transformer_kfold.py except:
  - MODEL_NAME = "microsoft/deberta-v3-base"  (184M vs 86M params)
  - EPOCHS = 2  (Exp 2 showed per-fold best is always ep2; smaller set overfits sooner)
  - OUTPUT_DIR = models/transformer_kfold_base

Run transformer_kfold_base_extract.py afterwards to produce ho_proba.npy /
test_proba.npy for stacking.py late fusion.
"""
...
if IS_KAGGLE:
    ...
    OUTPUT_DIR = Path("/kaggle/working/models/transformer_kfold_base")
else:
    ...
    DATA_DIR   = _root / "data"
    OUTPUT_DIR = _root / "models" / "transformer_kfold_base"
...
MAX_LENGTH    = 128
BATCH_SIZE    = 16
EPOCHS        = 2      # per-fold best is always ep2; base won't be different
FREEZE_EPOCHS = 0
CLS_DROPOUT   = 0.3
LR            = 2e-5
LLRD_FACTOR   = 0.9
...
model_slug        = "deberta-v3-base-kfold"
...
for fold_k, (tr_rel, val_rel) in enumerate(skf.split(tv_idx, tv_labels)):
    ...
    # LLRD optimizer (FREEZE_EPOCHS=0)
