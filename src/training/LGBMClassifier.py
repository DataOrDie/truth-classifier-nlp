...
wandb.login()
run = wandb.init(
    project="truth-classifier-lightgbm",
    config={
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_trainval, y_trainval), 1):
    ...
    if enable_hp_search and fold_best_params:
        ...
        _wandb_fold["hp/fold_subsample"]         = fold_best_params.get("subsample")
        _wandb_fold["hp/fold_colsample_bytree"]  = fold_best_params.get("colsample_bytree")
...
if enable_hp_search:
    ...
    wandb.log({
        ...
        "hp/final_num_leaves":        _best_params_final["num_leaves"],
...
# Feature importance
# -----------------------------------------------------------------------------
# LightGBM exposes split-count importance via feature_importances_ (default).
