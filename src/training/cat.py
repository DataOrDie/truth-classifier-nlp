...
# CatBoost native categorical support: pass integer-encoded categorical column
# indices so CatBoost applies ordered target statistics instead of treating them
# as numeric. This is CatBoost's core advantage over LGBM — it internally handles
# high-cardinality categoricals (interaction keys, speaker_grouped, etc.) without
# any loss of information from ordinal integer projection.
...
# HP search — nested CV: 5-fold outer, 3-fold inner RandomizedSearchCV.
# CatBoost-specific parameters:
#   iterations     — number of trees (equivalent to n_estimators)
#   depth          — max tree depth; CatBoost typically uses 6–10 (symmetric trees)
#   learning_rate  — shrinkage; same tradeoff as LGBM
#   l2_leaf_reg    — L2 regularisation on leaf values
#   border_count   — number of histogram bins for numeric features (higher = more splits)
#   bagging_temperature — controls Bayesian bootstrap randomness (0=no randomness, 1=standard)
...
for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_trainval, y_trainval), 1):
    ...
    if enable_hp_search and fold_best_params:
        _wandb_fold["hp/fold_iterations"]          = fold_best_params.get("iterations")
        _wandb_fold["hp/fold_learning_rate"]       = fold_best_params.get("learning_rate")
        _wandb_fold["hp/fold_depth"]               = fold_best_params.get("depth")
        _wandb_fold["hp/fold_l2_leaf_reg"]         = fold_best_params.get("l2_leaf_reg")
        _wandb_fold["hp/fold_border_count"]        = fold_best_params.get("border_count")
        _wandb_fold["hp/fold_bagging_temperature"] = fold_best_params.get("bagging_temperature")
...
if enable_hp_search:
    ...
    # All CatBoost HPs are discrete lists — pick the mode across folds
    for _hp in ["iterations", "learning_rate", "depth", "l2_leaf_reg",
    ...
    wandb.log({
        "hp/final_iterations":          _best_params_final["iterations"],
        "hp/final_learning_rate":       _best_params_final["learning_rate"],
        "hp/final_depth":               _best_params_final["depth"],
        "hp/final_l2_leaf_reg":         _best_params_final["l2_leaf_reg"],
        "hp/final_border_count":        _best_params_final["border_count"],
        "hp/final_bagging_temperature": _best_params_final["bagging_temperature"],
        "hp/search_table": wandb.Table(
            columns=["fold", "iterations", "learning_rate", "depth", "l2_leaf_reg",
                     "border_count", "bagging_temperature"],
            data=[
                [
                    m["fold"],
                    m["best_params"].get("iterations", "n/a"),
                    m["best_params"].get("learning_rate", "n/a"),
                    m["best_params"].get("depth", "n/a"),
                    m["best_params"].get("l2_leaf_reg", "n/a"),
                    m["best_params"].get("border_count", "n/a"),
                    m["best_params"].get("bagging_temperature", "n/a"),
                ]
                for m in cv_fold_metrics
            ],
        ),
...
# Feature importance
# -----------------------------------------------------------------------------
# CatBoost's default importance is PredictionValuesChange — equivalent to gain.
# get_feature_importance() returns values aligned to the feature order in the
# training pool; feature_names_ gives the corresponding column names.
...
axes[3].set_xlabel("PredictionValuesChange (gain)")
