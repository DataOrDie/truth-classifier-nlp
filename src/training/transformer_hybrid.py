"""
Transformer fine-tuning — Option B: hybrid text + metadata classification.

Architecture
------------
[statement tokens] → DeBERTa-v3-small → [CLS] (768-dim)  ──────────────────────────────┐
                                                                                           ├→ Dropout → Linear(784→2)
[4 meta features]  → BatchNorm → Linear(4→32) → GELU → Dropout → Linear(32→16) → GELU ──┘

Metadata features (computed fold-safe from train split only — no leakage):
  speaker_true_rate   — P(label==0 | speaker),         rare/unseen → global rate
  subject_true_rate   — P(label==0 | primary_subject),  rare/unseen → global rate
  party_true_rate     — P(label==0 | party_affiliation), rare/unseen → global rate
  is_major_party      — 1 if republican / democrat / none / democratic / independent

Why these features:
  speaker_true_rate is the single strongest predictor in the stacking ensemble.
  subject_true_rate adds topic-level credibility signal.
  party_true_rate is complementary (party affiliation alone is weaker, but as a rate it is clean).
  is_major_party distinguishes major-party speakers from fringe/unknown ones.
  speaker_job and state_info are excluded — too sparse (1090 / 78 unique) with low added signal.

Loss       : CrossEntropyLoss with class weights {0: 1.42, 1: 0.77}
Optimizer  : AdamW with LLRD — new heads get base_lr=2e-5, each DeBERTa layer ×0.9 deeper
Epochs     : 3  (best checkpoint saved by val macro_f1)
Max tokens : 128  (p99 of statement token length is ~41)
Precision  : FP32  (DeBERTa-v3 unstable in BF16)
"""
from datetime import datetime
from pathlib import Path
import sys
from time import time

import joblib
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import torch
import torch.nn as nn
import wandb
from sklearn.metrics import (
    accuracy_score,
    average_precision_score,
    balanced_accuracy_score,
    classification_report,
    confusion_matrix,
    f1_score,
    matthews_corrcoef,
    precision_score,
    recall_score,
    roc_auc_score,
    roc_curve,
    precision_recall_curve,
)
from sklearn.model_selection import train_test_split
from torch.amp import autocast
from torch.optim import AdamW
from torch.utils.data import DataLoader, Dataset
from transformers import (
    AutoModel,
    AutoTokenizer,
    get_linear_schedule_with_warmup,
)


# ============================================================
# Environment detection
# ============================================================
IS_KAGGLE = Path("/kaggle/input").exists()

if IS_KAGGLE:
    DATA_DIR   = Path("/kaggle/input/truth-classifier-nlp")
    OUTPUT_DIR = Path("/kaggle/working/models/transformer_hybrid")
else:
    def _find_root(start: Path) -> Path:
        for p in [start, *start.parents]:
            if (p / "data" / "train.csv").exists():
                return p
        raise FileNotFoundError("Cannot find project root")
    _root      = _find_root(Path.cwd())
    DATA_DIR   = _root / "data"
    OUTPUT_DIR = _root / "models" / "transformer_hybrid"

OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


# ============================================================
# Config
# ============================================================
MODEL_NAME     = "microsoft/deberta-v3-small"
MAX_LENGTH     = 128
BATCH_SIZE     = 16
EPOCHS         = 3
FREEZE_EPOCHS  = 0            # freeze DeBERTa encoder for this many epochs; 0 to disable
CLS_DROPOUT    = 0.3          # classifier head dropout (R7 validated; default ~0.1)
LR             = 2e-5        # head LR; DeBERTa layers decay by LLRD_FACTOR per layer
LLRD_FACTOR    = 0.9
WARMUP_RATIO   = 0.1
WEIGHT_DECAY   = 0.01
META_MIN_COUNT = 5            # min occurrences in train to use a speaker/subject/party rate

CLASS_WEIGHTS = [1.42, 0.77]  # {0: true, 1: false}
THRESHOLD     = 0.5
SEED          = 42

MAJOR_PARTIES = {"republican", "democrat", "none", "democratic", "independent"}

enable_threshold_tuning = True
create_kaggle_csv       = True
model_slug              = "deberta-v3-small-hybrid"

NUM_WORKERS = 0 if sys.platform == "win32" else 2

torch.manual_seed(SEED)
np.random.seed(SEED)

device  = torch.device("cuda" if torch.cuda.is_available() else "cpu")
USE_AMP = False

print(f"Device : {device}")
if device.type == "cuda":
    print(f"  GPU  : {torch.cuda.get_device_name(0)}")
    print(f"  VRAM : {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB")
print(f"  AMP  : {USE_AMP}")


# ============================================================
# Timing helper
# ============================================================
_script_start = time()
def _now() -> str:
    return datetime.now().strftime("%H:%M:%S")


# ============================================================
# Data loading
# ============================================================
print(f"\n[SECTION] Loading data  [{_now()}]")
df = pd.read_csv(DATA_DIR / "train.csv")
print(f"  Rows: {len(df):,}  |  Labels: {df['label'].value_counts().to_dict()}")

# Primary subject: first element of comma-separated multi-label string
df["subject_primary"] = df["subject"].str.split(",").str[0].str.strip()

print(f"\n[SECTION] Dataset features")
tok_len = df["statement"].str.split().str.len()
print(f"  Text input : 'statement'  token len min={tok_len.min()} median={tok_len.median():.0f} p99={tok_len.quantile(0.99):.0f} max={tok_len.max()}")
print(f"  Meta used  : speaker_true_rate | subject_true_rate | party_true_rate | is_major_party")
for col in ["speaker", "subject_primary", "party_affiliation", "speaker_job", "state_info"]:
    n_unique = df[col].nunique()
    top3 = df[col].value_counts().head(3).index.tolist()
    print(f"  {col:<22}: {n_unique:>4} unique  top-3={top3}")


# ============================================================
# 3-way split (index-based for fold-safe metadata)
# ============================================================
# Same seed + strategy as transformer.py so the holdout rows are identical
print(f"\n[SECTION] Splitting data  [{_now()}]")
idx_all    = np.arange(len(df))
labels_all = df["label"].values

idx_tv, idx_ho = train_test_split(
    idx_all, test_size=0.2, random_state=SEED, stratify=labels_all
)
idx_tr, idx_val = train_test_split(
    idx_tv, test_size=0.1, random_state=SEED, stratify=labels_all[idx_tv]
)

df_tr  = df.iloc[idx_tr].reset_index(drop=True)
df_val = df.iloc[idx_val].reset_index(drop=True)
df_ho  = df.iloc[idx_ho].reset_index(drop=True)
print(f"  Train: {len(df_tr):,}   Val: {len(df_val):,}   Holdout: {len(df_ho):,}")


# ============================================================
# Fold-safe metadata computation
# ============================================================
def build_rate_maps(df_train: pd.DataFrame) -> dict:
    """Build true-rate lookup maps from training data only. Save for inference."""
    global_rate = float((df_train["label"] == 0).mean())
    maps: dict = {"global_rate": global_rate}
    for col in ("speaker", "subject_primary", "party_affiliation"):
        counts = df_train[col].value_counts()
        valid  = counts[counts >= META_MIN_COUNT].index
        rate   = (df_train[df_train[col].isin(valid)]
                  .groupby(col)["label"]
                  .apply(lambda x: float((x == 0).mean()))
                  .to_dict())
        maps[col] = rate
    return maps


def build_meta(df_target: pd.DataFrame, maps: dict) -> np.ndarray:
    """Apply pre-built rate maps to any split. Shape: (N, 4)."""
    gr = maps["global_rate"]
    speaker_r = df_target["speaker"].map(maps["speaker"]).fillna(gr).astype(np.float32).values
    subject_r = df_target["subject_primary"].map(maps["subject_primary"]).fillna(gr).astype(np.float32).values
    party_r   = df_target["party_affiliation"].map(maps["party_affiliation"]).fillna(gr).astype(np.float32).values
    is_major  = df_target["party_affiliation"].str.lower().isin(MAJOR_PARTIES).astype(np.float32).values
    return np.stack([speaker_r, subject_r, party_r, is_major], axis=1)


print(f"\n[SECTION] Computing metadata features (fold-safe from train)  [{_now()}]")
rate_maps = build_rate_maps(df_tr)
meta_tr   = build_meta(df_tr,  rate_maps)
meta_val  = build_meta(df_val, rate_maps)
meta_ho   = build_meta(df_ho,  rate_maps)
N_META    = meta_tr.shape[1]

print(f"  Features : speaker_true_rate | subject_true_rate | party_true_rate | is_major_party")
print(f"  Shape    : {meta_tr.shape}")
print(f"  Train mean : {meta_tr.mean(axis=0).round(3)}")
print(f"  Train std  : {meta_tr.std(axis=0).round(3)}")


# ============================================================
# Tokenizer + Dataset
# ============================================================
print(f"\n[SECTION] Tokenizing  [{_now()}]")
tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)


class HybridDataset(Dataset):
    def __init__(self, texts: list[str], meta: np.ndarray, labels: list[int]):
        self.enc    = tokenizer(texts, truncation=True, padding="max_length",
                                max_length=MAX_LENGTH, return_tensors="pt")
        self.meta   = torch.tensor(meta, dtype=torch.float32)
        self.labels = torch.tensor(labels, dtype=torch.long)

    def __len__(self) -> int:
        return len(self.labels)

    def __getitem__(self, idx: int):
        return {k: v[idx] for k, v in self.enc.items()}, self.meta[idx], self.labels[idx]


_t0 = time()
train_ds   = HybridDataset(df_tr["statement"].tolist(),  meta_tr,  df_tr["label"].tolist())
val_ds     = HybridDataset(df_val["statement"].tolist(), meta_val, df_val["label"].tolist())
holdout_ds = HybridDataset(df_ho["statement"].tolist(),  meta_ho,  df_ho["label"].tolist())
print(f"  Tokenized in {time()-_t0:.1f}s")

_pin = USE_AMP
train_loader   = DataLoader(train_ds,   batch_size=BATCH_SIZE,   shuffle=True,
                            num_workers=NUM_WORKERS, pin_memory=_pin)
val_loader     = DataLoader(val_ds,     batch_size=BATCH_SIZE*2, shuffle=False,
                            num_workers=NUM_WORKERS, pin_memory=_pin)
holdout_loader = DataLoader(holdout_ds, batch_size=BATCH_SIZE*2, shuffle=False,
                            num_workers=NUM_WORKERS, pin_memory=_pin)


# ============================================================
# Hybrid model
# ============================================================
class HybridDeBERTa(nn.Module):
    """DeBERTa [CLS] + metadata MLP → concat → classifier."""

    def __init__(self, n_meta: int):
        super().__init__()
        self.encoder = AutoModel.from_pretrained(MODEL_NAME, torch_dtype=torch.float32)
        hidden = self.encoder.config.hidden_size  # 768 for deberta-v3-small

        self.meta_mlp = nn.Sequential(
            nn.BatchNorm1d(n_meta),
            nn.Linear(n_meta, 32),
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(32, 16),
            nn.GELU(),
        )
        self.dropout    = nn.Dropout(CLS_DROPOUT)
        self.classifier = nn.Linear(hidden + 16, 2)

    def forward(self, enc_inputs: dict, meta: torch.Tensor) -> torch.Tensor:
        cls     = self.encoder(**enc_inputs).last_hidden_state[:, 0, :]
        meta_out = self.meta_mlp(meta)
        return self.classifier(self.dropout(torch.cat([cls, meta_out], dim=-1)))


# ============================================================
# LLRD optimizer builder
# ============================================================
def _build_llrd_param_groups(model: HybridDeBERTa,
                              base_lr: float, llrd_factor: float,
                              weight_decay: float) -> list:
    """
    Head (meta_mlp + classifier) : base_lr
    DeBERTa encoder layer 11     : base_lr * factor^1
    DeBERTa encoder layer 10     : base_lr * factor^2
    ...
    DeBERTa encoder layer 0      : base_lr * factor^12
    Embeddings + rel_embeddings  : base_lr * factor^13
    """
    no_decay   = {"bias", "LayerNorm.weight", "layer_norm.weight"}
    num_layers = len(model.encoder.encoder.layer)
    param_dict = dict(model.named_parameters())
    assigned   = set()
    groups     = []

    def _add(names: list[str], lr: float) -> None:
        wd = [param_dict[n] for n in names if not any(nd in n for nd in no_decay)]
        nd = [param_dict[n] for n in names if     any(nd in n for nd in no_decay)]
        if wd: groups.append({"params": wd, "lr": lr, "weight_decay": weight_decay})
        if nd: groups.append({"params": nd, "lr": lr, "weight_decay": 0.0})
        assigned.update(names)

    # New heads: meta_mlp.*, classifier.*  (anything not under the encoder module)
    _add([n for n in param_dict if not n.startswith("encoder.")], base_lr)

    # DeBERTa encoder layers: top → bottom
    for layer_idx in range(num_layers - 1, -1, -1):
        depth = num_layers - layer_idx          # 1 for layer 11, num_layers for layer 0
        lr    = base_lr * (llrd_factor ** depth)
        _add([n for n in param_dict if f"encoder.encoder.layer.{layer_idx}." in n], lr)

    # Everything else (embeddings, rel_embeddings, etc.) — lowest LR
    embed_lr = base_lr * (llrd_factor ** (num_layers + 1))
    _add([n for n in param_dict if n not in assigned], embed_lr)

    return groups


# ============================================================
# Freeze / unfreeze helpers
# ============================================================
def _freeze_backbone(model: HybridDeBERTa) -> None:
    frozen = trainable = 0
    for name, param in model.named_parameters():
        if name.startswith("encoder."):
            param.requires_grad_(False)
            frozen += param.numel()
        else:
            trainable += param.numel()
    print(f"  Backbone frozen — frozen={frozen:,}  trainable={trainable:,}")


def _unfreeze_backbone(model: HybridDeBERTa) -> None:
    for param in model.parameters():
        param.requires_grad_(True)
    print(f"  Backbone unfrozen — trainable={sum(p.numel() for p in model.parameters()):,}")


# ============================================================
# Model + optimizer + scheduler
# ============================================================
print(f"\n[SECTION] Loading model: {MODEL_NAME}  [{_now()}]")
model = HybridDeBERTa(n_meta=N_META).to(device)
n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"  Parameters : {n_params:,}")
print(f"  Architecture: DeBERTa({MODEL_NAME}) [CLS](768) + MetaMLP({N_META}→32→16) → Linear(784→2)")

loss_weights = torch.tensor(CLASS_WEIGHTS, dtype=torch.float32).to(device)
criterion    = nn.CrossEntropyLoss(weight=loss_weights)

# ---- Phase 1: meta_mlp + classifier only (frozen backbone) ----
if FREEZE_EPOCHS > 0:
    _freeze_backbone(model)
p1_steps  = len(train_loader) * max(FREEZE_EPOCHS, 1)
p1_warmup = int(p1_steps * WARMUP_RATIO)
p1_params = [p for p in model.parameters() if p.requires_grad]
optimizer = AdamW(p1_params, lr=LR, weight_decay=WEIGHT_DECAY)
scheduler = get_linear_schedule_with_warmup(optimizer, p1_warmup, p1_steps)
print(f"  Phase 1 optimizer — {len(p1_params)} param tensors  lr={LR:.1e}")

# ---- Phase 2: full LLRD (built lazily at epoch FREEZE_EPOCHS+1) ----
p2_steps  = len(train_loader) * max(EPOCHS - FREEZE_EPOCHS, 0)
p2_warmup = int(p2_steps * WARMUP_RATIO)


# ============================================================
# W&B
# ============================================================
print("\n[SECTION] Initializing W&B run")
wandb.login()
run = wandb.init(
    project="truth-classifier-transformers",
    config={
        "model":         MODEL_NAME,
        "architecture":  "hybrid-text+meta",
        "n_meta":        N_META,
        "meta_features": ["speaker_true_rate", "subject_true_rate",
                          "party_true_rate", "is_major_party"],
        "max_length":    MAX_LENGTH,
        "batch_size":    BATCH_SIZE,
        "epochs":        EPOCHS,
        "freeze_epochs": FREEZE_EPOCHS,
        "cls_dropout":   CLS_DROPOUT,
        "lr":            LR,
        "llrd_factor":   LLRD_FACTOR,
        "warmup_ratio":  WARMUP_RATIO,
        "weight_decay":  WEIGHT_DECAY,
        "class_weights": CLASS_WEIGHTS,
        "scheduler":     "linear",
        "seed":          SEED,
        "n_train":       len(df_tr),
        "n_val":         len(df_val),
        "n_holdout":     len(df_ho),
        "use_amp":       USE_AMP,
        "device":        str(device),
    },
)


# ============================================================
# Training helpers
# ============================================================
def train_epoch(model, loader, optimizer, scheduler, criterion) -> float:
    model.train()
    total_loss = 0.0
    for batch_idx, (enc_inputs, meta, labs) in enumerate(loader):
        enc_inputs = {k: v.to(device) for k, v in enc_inputs.items()}
        meta       = meta.to(device)
        labs       = labs.to(device)
        optimizer.zero_grad()
        with autocast("cuda", dtype=torch.bfloat16, enabled=USE_AMP):
            logits = model(enc_inputs, meta)
            loss   = criterion(logits, labs)
        loss.backward()
        nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        optimizer.step()
        scheduler.step()
        total_loss += loss.item()
        if batch_idx == 0:
            print(f"    Batch 0 — logits dtype={logits.dtype}  labs dtype={labs.dtype}  loss={loss.item():.4f}")
        if (batch_idx + 1) % 50 == 0:
            print(f"    Batch {batch_idx+1}/{len(loader)}  avg_loss={total_loss/(batch_idx+1):.4f}")
    return total_loss / len(loader)


@torch.no_grad()
def evaluate(model, loader) -> tuple[float, np.ndarray, np.ndarray]:
    model.eval()
    all_logits, all_labels, total_loss = [], [], 0.0
    for enc_inputs, meta, labs in loader:
        enc_inputs = {k: v.to(device) for k, v in enc_inputs.items()}
        meta       = meta.to(device)
        with autocast("cuda", dtype=torch.bfloat16, enabled=USE_AMP):
            logits = model(enc_inputs, meta)
            loss   = criterion(logits, labs.to(device))
        total_loss += loss.item()
        all_logits.append(logits.float().cpu())
        all_labels.append(labs)
    logits = torch.cat(all_logits)
    labels = torch.cat(all_labels).numpy()
    proba  = torch.softmax(logits, dim=-1)[:, 1].numpy()
    return total_loss / len(loader), proba, labels


# ============================================================
# Training loop
# ============================================================
print(f"\n[SECTION] Training  [{_now()}]")
print(f"  Model dtype     : {next(model.parameters()).dtype}")
print(f"  loss_weights    : {loss_weights}")
print(f"  Train batches   : {len(train_loader)}  Val batches: {len(val_loader)}")

best_val_f1 = -1.0
best_ckpt   = OUTPUT_DIR / f"{model_slug}-best.pt"

for epoch in range(1, EPOCHS + 1):
    _t = time()
    # Switch to phase 2 (LLRD, full backbone) on the first post-freeze epoch
    if FREEZE_EPOCHS > 0 and epoch == FREEZE_EPOCHS + 1:
        print(f"\n  [Phase 2] Unfreezing backbone + rebuilding optimizer with LLRD")
        _unfreeze_backbone(model)
        param_groups  = _build_llrd_param_groups(model, LR, LLRD_FACTOR, WEIGHT_DECAY)
        optimizer     = AdamW(param_groups)
        _lr_min = min(g["lr"] for g in param_groups)
        _lr_max = max(g["lr"] for g in param_groups)
        print(f"  LLRD groups: {len(param_groups)}  LR range: [{_lr_min:.2e}, {_lr_max:.2e}]")
        scheduler = get_linear_schedule_with_warmup(optimizer, p2_warmup, p2_steps)
    print(f"\n  --- Epoch {epoch}/{EPOCHS} ---  [{_now()}]")
    train_loss = train_epoch(model, train_loader, optimizer, scheduler, criterion)
    print(f"  Train loss: {train_loss:.4f}  — starting val evaluation")
    val_loss, val_proba, val_labels = evaluate(model, val_loader)
    print(f"  Val proba range: [{val_proba.min():.4f}, {val_proba.max():.4f}]  NaNs: {np.isnan(val_proba).sum()}")

    val_pred = (val_proba >= 0.5).astype(int)
    val_f1   = f1_score(val_labels, val_pred, average="macro", zero_division=0)
    val_auc  = roc_auc_score(val_labels, val_proba)

    print(
        f"  Epoch {epoch}/{EPOCHS}  "
        f"train_loss={train_loss:.4f}  val_loss={val_loss:.4f}  "
        f"val_macro_f1={val_f1:.4f}  val_roc_auc={val_auc:.4f}  "
        f"({time()-_t:.1f}s)"
    )
    wandb.log({"epoch": epoch, "train/loss": train_loss,
               "val/loss": val_loss, "val/macro_f1": val_f1, "val/roc_auc": val_auc})

    if val_f1 > best_val_f1:
        best_val_f1 = val_f1
        torch.save(model.state_dict(), best_ckpt)
        print(f"    New best val macro_f1={best_val_f1:.4f} — checkpoint saved")


# ============================================================
# Load best checkpoint
# ============================================================
print(f"\n[SECTION] Loading best checkpoint  [{_now()}]")
model.load_state_dict(torch.load(best_ckpt, map_location=device))


# ============================================================
# Threshold tuning on val set
# ============================================================
_, val_proba, val_labels = evaluate(model, val_loader)

if enable_threshold_tuning:
    print(f"\n[SECTION] Threshold tuning on val set  [{_now()}]")
    grid   = np.arange(0.20, 0.76, 0.01)
    scores = {
        round(float(t), 2): f1_score(val_labels, (val_proba >= t).astype(int),
                                     average="macro", zero_division=0)
        for t in grid
    }
    best_t = max(scores, key=scores.get)
    print(f"  {'threshold':>10}   macro_f1")
    for t, s in scores.items():
        print(f"  {t:>10.2f}   {s:.4f}{'  ←' if t == best_t else ''}")
    print(f"\n  Best threshold: {best_t:.2f}  (val macro_f1={scores[best_t]:.4f})")
    wandb.log({"threshold/best": best_t, "threshold/val_macro_f1": scores[best_t]})
    THRESHOLD = best_t


# ============================================================
# Holdout evaluation
# ============================================================
print(f"\n[SECTION] Holdout evaluation  [{_now()}]")
print(f"  Threshold: {THRESHOLD:.2f}")

_, ho_proba, ho_labels = evaluate(model, holdout_loader)
ho_pred = (ho_proba >= THRESHOLD).astype(int)

holdout_metrics = {
    "roc_auc":      roc_auc_score(ho_labels, ho_proba),
    "pr_auc":       average_precision_score(ho_labels, ho_proba),
    "macro_f1":     f1_score(ho_labels, ho_pred, average="macro", zero_division=0),
    "f1":           f1_score(ho_labels, ho_pred, zero_division=0),
    "precision":    precision_score(ho_labels, ho_pred, zero_division=0),
    "recall":       recall_score(ho_labels, ho_pred, zero_division=0),
    "accuracy":     accuracy_score(ho_labels, ho_pred),
    "mcc":          matthews_corrcoef(ho_labels, ho_pred),
    "balanced_acc": balanced_accuracy_score(ho_labels, ho_pred),
}
cm = confusion_matrix(ho_labels, ho_pred)

print("\nHoldout results:")
for name, val in holdout_metrics.items():
    print(f"  {name}: {val:.4f}")
print(f"\n{classification_report(ho_labels, ho_pred)}")


# ============================================================
# Plots + W&B logging
# ============================================================
print("\n[SECTION] Generating plots")
fpr, tpr, _      = roc_curve(ho_labels, ho_proba)
prec_c, rec_c, _ = precision_recall_curve(ho_labels, ho_proba)

fig, axes = plt.subplots(1, 3, figsize=(18, 5))
axes[0].plot(fpr, tpr, label=f"ROC-AUC = {holdout_metrics['roc_auc']:.4f}")
axes[0].plot([0, 1], [0, 1], "k--", alpha=0.6)
axes[0].set(title=f"ROC Curve — {model_slug} (holdout)", xlabel="FPR", ylabel="TPR")
axes[0].legend()
axes[1].plot(rec_c, prec_c, label=f"PR-AUC = {holdout_metrics['pr_auc']:.4f}")
axes[1].set(title="Precision-Recall Curve (holdout)", xlabel="Recall", ylabel="Precision")
axes[1].legend()
im = axes[2].imshow(cm, interpolation="nearest", cmap="Blues")
axes[2].set_title("Confusion Matrix (holdout)")
axes[2].set_xticks([0, 1]); axes[2].set_xticklabels(["True (0)", "False (1)"])
axes[2].set_yticks([0, 1]); axes[2].set_yticklabels(["True (0)", "False (1)"])
for i in range(2):
    for j in range(2):
        axes[2].text(j, i, str(cm[i, j]), ha="center", va="center", color="black")
fig.colorbar(im, ax=axes[2])
plt.tight_layout()

wandb.log({
    **{f"holdout/{k}": v for k, v in holdout_metrics.items()},
    "holdout/tn": int(cm[0, 0]), "holdout/fp": int(cm[0, 1]),
    "holdout/fn": int(cm[1, 0]), "holdout/tp": int(cm[1, 1]),
    "roc_pr_cm": wandb.Image(fig),
    "confusion_matrix": wandb.plot.confusion_matrix(
        y_true=ho_labels.tolist(), preds=ho_pred.tolist(),
        class_names=["True (0)", "False (1)"],
    ),
})
run.summary["holdout/macro_f1"] = holdout_metrics["macro_f1"]
run.summary["holdout/roc_auc"]  = holdout_metrics["roc_auc"]

print("\n[SECTION] Finishing W&B run")
run.finish()


# ============================================================
# Save artifacts
# ============================================================
print(f"\n[SECTION] Saving artifacts  [{_now()}]")
torch.save(model.state_dict(), OUTPUT_DIR / f"{model_slug}-weights.pt")
tokenizer.save_pretrained(OUTPUT_DIR / f"{model_slug}-tokenizer")
joblib.dump({
    "rate_maps":  rate_maps,
    "threshold":  THRESHOLD,
    "n_meta":     N_META,
    "major_parties": list(MAJOR_PARTIES),
}, OUTPUT_DIR / f"{model_slug}-meta.joblib")
print(f"  Saved to: {OUTPUT_DIR}")


# ============================================================
# Kaggle submission CSV
# ============================================================
if create_kaggle_csv:
    print(f"\n[SECTION] Creating Kaggle submission  [{_now()}]")
    df_test = pd.read_csv(DATA_DIR / "test_nolabel.csv")
    df_test["subject_primary"] = df_test["subject"].str.split(",").str[0].str.strip()

    meta_test  = build_meta(df_test, rate_maps)
    test_texts = df_test["statement"].tolist()

    test_enc = tokenizer(
        test_texts, truncation=True, padding="max_length",
        max_length=MAX_LENGTH, return_tensors="pt",
    )
    test_meta = torch.tensor(meta_test, dtype=torch.float32)

    model.eval()
    all_proba = []
    _bsz = BATCH_SIZE * 2
    with torch.no_grad():
        for i in range(0, len(test_texts), _bsz):
            batch_enc  = {k: v[i:i + _bsz].to(device) for k, v in test_enc.items()}
            batch_meta = test_meta[i:i + _bsz].to(device)
            with autocast("cuda", dtype=torch.bfloat16, enabled=USE_AMP):
                logits = model(batch_enc, batch_meta)
            all_proba.append(torch.softmax(logits.float(), dim=-1)[:, 1].cpu().numpy())

    test_proba = np.concatenate(all_proba)
    test_pred  = (test_proba >= THRESHOLD).astype(int)

    sub_dir = Path("/kaggle/working") if IS_KAGGLE else (_root / "submissions")
    sub_dir.mkdir(exist_ok=True)
    sub_path = sub_dir / f"submission-{model_slug}-{datetime.now().strftime('%Y%m%d-%H%M')}.csv"
    pd.DataFrame({"id": df_test["id"], "label": test_pred}).to_csv(sub_path, index=False)
    print(f"  Submission saved: {sub_path}  ({len(test_pred):,} rows)")

print(f"\n[DONE] Total time: {time()-_script_start:.1f}s  [{_now()}]")
