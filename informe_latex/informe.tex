...
\begin{titlepage}
  ...
  {\normalsize
  \begin{tabular}{c}
    Iv\'{a}n Omar Angeles Mart\'{i}nez       \\[0.3em]
...
Los modelos Transformer mostraron mejores resultados que los modelos clásicos cuando se combinaron con información contextual. Sin embargo, el mejor rendimiento global se obtuvo mediante estrategias híbridas de \emph{late fusion}, donde las predicciones de DeBERTa~\cite{deberta} se integraban dentro del \emph{stacking} junto con modelos \emph{ensemble} tradicionales.

El mejor modelo del proyecto fue una combinación de DeBERTa-v3-base y \emph{stacking}, alcanzando los valores más altos tanto en Macro F1 como en ROC-AUC.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \textbf{\emph{Late Fusion} + DeBERTa-v3-base}                    & \textbf{0,647}    & \textbf{0,711}   & ---  \\
    \bottomrule
  \end{tabular}
\end{table}
...
Los modelos Transformer~\cite{hf_transformers,deberta} mejoraron notablemente al incorporar \emph{metadata} contextual directamente en el texto de entrada. Sin embargo, los mejores resultados no se obtuvieron con modelos \emph{standalone}, sino mediante estrategias híbridas de \emph{late fusion} que combinaban las fortalezas de los Transformers y los modelos \emph{ensemble} tradicionales~\cite{lgbm,catboost,xgboost}.

En general, los resultados muestran que la combinación de distintas representaciones textuales y enfoques de modelado permitió obtener un sistema más robusto y con mejor capacidad de generalización que cualquiera de los modelos individuales por separado.
...
El mejor resultado del proyecto se alcanzó mediante una estrategia híbrida de \emph{late fusion} combinando DeBERTa-v3-base~\cite{deberta} con \emph{stacking} de modelos \emph{ensemble}, obteniendo los valores más altos de Macro F1 y ROC-AUC. Esto demuestra que la combinación de modelos con mecanismos de aprendizaje diferentes puede aprovechar mejor las fortalezas de cada enfoque y mejorar la capacidad de generalización final del sistema.

En conclusión, el proyecto permitió comparar distintas técnicas de clasificación textual y comprobar cómo la integración de información contextual y estrategias híbridas puede mejorar significativamente el rendimiento en tareas de detección de desinformación política.
